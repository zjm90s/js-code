...
function printResult() {
	$this = $(".tb-revbd");
	$this.html("");
	$this.append('<div style="margin: 10px 0px 0px 40px; text-align: left;"><b>用户名</b> # <b>等级</b> # <b>评论时间</b> # <b>评论字数</b> # <b>购买次数</b> # <b>评分</b></div>');
	var count = 0;
	for (var key in cmtMap) {
		...
		for (var i = 0; i < cmtMap[key].length; i++) {
			...
			$this.append('<div style="margin: 15px 0px 0px 60px; text-align: left; "><b>'
				+comment.user+'</b> # <b>'+lvMap[comment.level][0]+'</b> # <b>'+comment.time+'</b> # <b>'+comment.txtLength+'</b> # <b>'+comment.buyTimes+'</b> # <b>'+comment.score+'</b></div>');
